      ...
      <div className="md-grid">
        <div className="md-cell md-cell--12 md-cell--8-tablet md-cell--4-phone">
          ...
          <ul style={{ listStyle: "none", textAlign: "center" }}>
            {_.map(t("description1", { returnObjects: true }), (item, key) => (
              <Styledli key={key}> {item}</Styledli>
            ))}
          </ul>
        </div>
      </div>
      <div className="md-grid">
        <div className="md-cell md-cell--6 md-cell--4-tablet md-cell--4-phone">
          ...
          <ul style={{ textAlign: "left" }}>
            {_.map(t("description2", { returnObjects: true }), (item, key) => (
              <Styledli key={key}> {item}</Styledli>
            ))}
          </ul>
        </div>
        <div className="md-cell md-cell--6 md-cell--4-tablet md-cell--4-phone">
          ...
          <ul style={{ textAlign: "left" }}>
            {_.map(t("description3", { returnObjects: true }), (item, key) => (
              <Styledli key={key}> {item}</Styledli>
            ))}
          </ul>
        </div>
